...
void loop() {
  ...
  // if greenManPin is set to flash and redPin hasn't been on longer than redToAmberDelay time - redManToAmberDelay time and greenManFlashDelay time has passed since last flip
  // flip greenManPin state (flash)
  if (stage == 7 && greenManFlashState == true && !(currentMillis - redPreviousMillis >= redToAmberDelay - redManToAmberDelay) && currentMillis - greenManFlashPreviousMillis >= greenManFlashDelay){
  ...
  // if amberPin is set to flash and amberFlashDelay time has passed since last flip
  // flip state of amberPin (flash)
  if (stage == 8 && amberFlashState == true && currentMillis - flashingAmberPreviousMillis >= amberFlashDelay){
